...
class TodoList:
    ...
    def add_task(self, title, priority="средний"):
    ...
    def show_tasks(self, show_completed=True):
        ...
        if pending:
            ...
            for task in pending:
                ...
                print(
                    f"[{task['id']}] {priority_icon.get(task['priority'], '⚪')} "
                    f"{task['title']}"
                )
    ...
    while True:
        ...
        if choice == "1":
            ...
            if title:
                ...
                prior_choice = input(
                    "Выберите приоритет (по умолчанию средний): "
                ).strip()
            ...
            print(f"({completed * 100 // total if total > 0 else 0}%)")
